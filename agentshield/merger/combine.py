...
def _render_how_it_works(parts: list[str]) -> None:
    ...
    parts.append(
        ...
        'available. The rule pack bundled with AgentShield carries '
        '<em>both</em> the static patterns Phase 1 fires and the '
        'runtime payloads Phase 2 sends &mdash; same source, different '
        'execution mode.</p>'
    )
    ...
    parts.append(
        ...
        'Copilot-shaped backend gets payload + response + rule context, '
        'returns verdict + plain-text reasoning + <code>confidence</code> '
        '(0&hellip;1). LLM verdict wins the headline; heuristic verdict '
        'is recorded for reference. Swappable for real boto3-Bedrock '
        'with one class change.</li>'
