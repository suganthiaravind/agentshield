...
def _render_emulator_reference_body(parts: list[str]) -> None:
    ...
    parts.append(
        ...
        '<tr>'
        '<td><strong>&rarr;store</strong> '
        '<em class="emu-ref-type-badge">live stack</em></td>'
        '<td>Can untrusted data be written to a persistent store '
        '(vector DB, memory, cache) and retrieved in a future session?</td>'
        '<td>Memory poisoning, indirect injection via persisted content. '
        '<em style="color:#94a3b8;font-size:10px">'
        'Requires a real persistent backend to fully verify.</em></td>'
    ...
    parts.append(
        ...
        '<p style="margin-bottom:8px">Each source &times; transition result records which payload finally '
