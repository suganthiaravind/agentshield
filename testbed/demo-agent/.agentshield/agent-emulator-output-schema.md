...
| `verdict_reasoning` | string | One-paragraph explanation citing the load-bearing file:line evidence. |
| `frameworks` | object | OWASP / ATLAS / CWE mappings — same shape as Tier 1 findings so the renderer reuses framework-chip styling. |
| `pipeline_trace` | array | One entry per targeted step, in pipeline-walk order. See below. |
